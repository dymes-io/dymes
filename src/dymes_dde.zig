...
const ParsedState = struct {
    ...
    pub fn init(gpa: std.mem.Allocator, dymes_config: Config, query: Query, filters: []Filter, output_path: []const u8, gen_data: bool, append_flag: bool) CreationError!ParsedState {
        const owned_filters = gpa.dupe(Filter, filters) catch return CreationError.OutOfMemory;
        errdefer gpa.free(owned_filters);

        var engine_config_builder = try config.builder("dymes-data-exporter", gpa);
...
pub fn main() !void {
    ...
    var data_exporter_config = try parseCmdLine(allocator);
    defer data_exporter_config.deinit(allocator);
    ...
    var logger = logging.logger("dde");

    logger.debug().msg("Dymes Data Exporter Tool initializing").log();

    const client_config = try data_exporter_config.config.asConfig("exporter.client") orelse return UsageError.OtherUsageFailure;
    ...
    if (data_exporter_config.gen_data) {
        _ = try generateTestData(logger, allocator, &client);
    }

    logger.info().msg("Dymes Data Exporter Tool initialized").log();
    try runDataExporterTool(logger, allocator, data_exporter_config, &client);
    logger.info().msg("Dymes Data Exporter Tool terminating").log();
}
...
fn parseCmdLine(allocator: Allocator) !ParsedState {
    const params = comptime clap.parseParamsComptime(
        ...
        \\
        \\-o, --output <str>       Specifiy the path where the data will be written to. If left blank, output will be printed to std out.
        \\
        \\--gentestdata            Generate test data before running queries. Used for testing and debugging.
        \\
        \\-a, --append             Flag to specify if you want the results appended to the output file instead of overwriting/creating from scratch.
        \\
    ...
    const config_path = res.args.config orelse "conf/dymes-data-exporter.yaml";
    ...
    if (try dymes_config.asString("data-exporter.logging.level")) |logging_level_name| {
    ...
    if (try dymes_config.asString("data-exporter.logging.format")) |logging_format_name| {
...
fn runDataExporterTool(logger: *logging.Logger, allocator: Allocator, parsedState: ParsedState, client: *Client) !void {
    logger.debug().msg("Running data exporter").log();
    const query = parsedState.query;

    const client_query: ClientQueryDto = switch (query) {
        .query_single => |msg_id| blk: {
            const enc = try msg_id.encodeAlloc(allocator);
            break :blk .{
                .query_dto = .{
                    .query_single = enc,
                },
        ...
        .query_range => |r| blk: {
            const start = try r.range_start.encodeAlloc(allocator);
            errdefer allocator.free(start);
            const end = try r.range_end.encodeAlloc(allocator);
        ...
        .query_correlation => |cid| blk: {
            const enc = try cid.encodeAlloc(allocator);
            break :blk .{
                .query_dto = .{
                    .query_correlation = enc,
                },
                .filters = parsedState.filters,
            };
        },
    ...
    defer switch (client_query.query_dto) {
        .query_single => |x| allocator.free(x),
        .query_range => |r| {
            allocator.free(r.range_start);
            allocator.free(r.range_end);
        },
        .query_correlation => |x| allocator.free(x),
        else => {},
    };

    var cursor = try client.openCursor(client_query, .{ .batch_size = 30 }) orelse return error.NoResults;
    defer cursor.close();

    const write_buffer = try allocator.alloc(u8, 4096);
    defer allocator.free(write_buffer);
    if (parsedState.output_path.len > 0) {
        if (parsedState.append_flag) {
            var out_file = try std.fs.cwd().createFile(parsedState.output_path, .{ .truncate = false });
            defer out_file.close();
            _ = try out_file.seekFromEnd(0);
            var raw_writer = out_file.writer(write_buffer);
            var writer = &raw_writer.interface;
            ...
            while (try cursor.next()) |_message| {
                ...
                try _message.renderAsJSON(writer);
            }
            try writer.writeAll("]");
        } else {
            var out_file = try std.fs.cwd().createFile(parsedState.output_path, .{});
            defer out_file.close();
            var raw_writer = out_file.writer(write_buffer);
            var writer = &raw_writer.interface;
            defer writer.flush() catch {};
...
fn buildBaseConfig(allocator: std.mem.Allocator, config_path: []const u8) ConfigError!Config {
    var config_bld = try config.builder("dymes_data_exporter", allocator);
...
fn generateTestData(logger: *logging.Logger, allocator: std.mem.Allocator, client: *Client) !FirstAndLast {
    ...
    while (i < 100) : (i += 1) {
        ...
        if (i == 0) {
            firstAndLast.first = try client.appendMessage(body, channel, routing, kv, null);
        } else if (i == 99) {
            firstAndLast.last = try client.appendMessage(body, channel, routing, kv, null);
        } else {
            _ = try client.appendMessage(body, channel, routing, kv, null);
        }
    }
    logger.info()
...
fn wrapper(realTest: *const fn (ctx: TestCtx) anyerror!void, filters: []const []const u8, query: Query) !void {
    ...
    const logger = logging.logger("test.dde");

    var tmp_dir = std.testing.tmpDir(.{ .iterate = true });
    defer tmp_dir.cleanup();
    try tmp_dir.dir.makeDir("data-exporter-sanity-check");
    const abs_dir_name = try tmp_dir.dir.realpathAlloc(allocator, "data-exporter-sanity-check");
    ...
    try cfg_bld.withInt("engine.http.port", 6511);
    ...
    var engine = try Engine.init(allocator, frame_allocator, &health, engine_config);
    defer engine.deinit();

    const http_config = try engine_config.asConfig("http") orelse unreachable;

    var server = try Server.init(allocator, engine, .{
        .health = &health,
    ...
    try cfg_bld_client.withInt("client.port", 6511);
    ...
    logger.info()
        .msg("client data")
        .any("client config", client_config)
        .log();
...
test "checkGenerateTestsCreates100Entries" {
    const filters = &[_][]const u8{};
    const query = Query.range(min_ulid, max_ulid);
    try wrapper(&checkGenerateTestsCreates100EntriesRealTest, filters, query);
}

fn checkGenerateTestsCreates100EntriesRealTest(ctx: TestCtx) !void {
    _ = try generateTestData(ctx.logger, ctx.allocator, ctx.client);

    const client_query: ClientQueryDto = switch (ctx.query) {
        .query_single => |msg_id| blk: {
            const enc = try msg_id.encodeAlloc(ctx.allocator);
            break :blk .{
                .query_dto = .{
                    .query_single = enc,
                },
        ...
        .query_correlation => |cid| blk: {
            const enc = try cid.encodeAlloc(ctx.allocator);
            break :blk .{
                .query_dto = .{
                    .query_correlation = enc,
                },
    ...
    defer switch (client_query.query_dto) {
        .query_single => |x| ctx.allocator.free(x),
        .query_range => |r| {
            ctx.allocator.free(r.range_start);
            ctx.allocator.free(r.range_end);
        },
        .query_correlation => |x| ctx.allocator.free(x),
        else => {},
    };
...
fn singlequeryRealTest(ctx: TestCtx) !void {
    const firstAndLast = try generateTestData(ctx.logger, ctx.allocator, ctx.client);
    const real_query = Query.single(firstAndLast.first);

    const client_query: ClientQueryDto = switch (real_query) {
        .query_single => |msg_id| blk: {
            const enc = try msg_id.encodeAlloc(ctx.allocator);
            break :blk .{
                .query_dto = .{
...
fn singlequeryinvalidRealTest(ctx: TestCtx) !void {
    _ = try generateTestData(ctx.logger, ctx.allocator, ctx.client);
    const real_query = Query.single(max_ulid);
...
fn rangequeryRealTest(ctx: TestCtx) !void {
    ...
    const client_query: ClientQueryDto = switch (realQuery) {
        .query_single => |msg_id| blk: {
            const enc = try msg_id.encodeAlloc(ctx.allocator);
            break :blk .{
                .query_dto = .{
