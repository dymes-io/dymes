//! Message creation request builder.
//!
...
const CreationRequest = @import("CreationRequest.zig");

const component_name = "msg.CreationRequestBuilder";
...
pub fn build(self: *Self) BuildError!CreationRequest {
    ...
    return CreationRequest.init(encoded_correlation_id, self.channel.?, self.routing.?, self.encoded_msg_body.?, kv_entries);
}
...
const CreationRequestBuilder = @This();

test "CreationRequestBuilder" {
    std.testing.refAllDeclsRecursive(@This());

    std.debug.print("test.CreationRequestBuilder\n", .{});
    ...
    // var logger = logging.logger("test.CreationRequestBuilder");

    var req_bld = try CreationRequestBuilder.init(allocator);
