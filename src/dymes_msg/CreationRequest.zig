...
const component_name = "msg.CreationRequest";
...
pub fn init(encoded_correlation_id: ?EncodedUlid, channel: u128, routing: u128, encoded_msg_body: []const u8, kv_entries: []const KvPair) AllocationError!Self {
    return .{
...
test "CreationRequest" {
    std.testing.refAllDeclsRecursive(@This());

    std.debug.print("test.CreationRequest\n", .{});
    ...
    var logger = logging.logger("test.CreationRequest");
    ...
    const creation_request = try CreationRequest.init(encoded_correlation, 702, 101, encoded_body, &kv_entries);
    // defer creation_request.deinit(allocator);

    {
        ...
        try std.json.Stringify.value(creation_request, .{ .whitespace = .minified }, writer);

        logger.debug()
            .msg("JSON encoding")
            .str("creation_request_json", arr_json.items)
            .log();

        const parsed = try std.json.parseFromSlice(CreationRequest, allocator, arr_json.items, .{});
        defer parsed.deinit();

        logger.debug()
            .msg("JSON decoding")
            .any("creation_request", parsed.value)
            .log();
        try testing.expectEqual(creation_request.channel, parsed.value.channel);
        try testing.expectEqual(creation_request.routing, parsed.value.routing);
        ...
        try testing.expectEqualStrings(kv_entries[0].key, creation_request.kv_entries[0].key);
        try testing.expectEqualStrings(kv_entries[0].value, creation_request.kv_entries[0].value);
        try testing.expectEqualStrings(kv_entries[1].key, creation_request.kv_entries[1].key);
        try testing.expectEqualStrings(kv_entries[1].value, creation_request.kv_entries[1].value);
    }
}
